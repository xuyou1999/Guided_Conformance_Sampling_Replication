...
class FeatureGuidedLogSampler:
    def __init__(self, use_cache=True, alignment_cache={}, preprocessing_time=None, index_file=None):
    ...
    def explore(self, log, verbose=False):
        #e_time = time.time()
        #print("Explore")
        sampled_trace = random.choice([idx for idx in [*range(len(log))] if idx not in self.sample.traces])
    ...
    def construct_sample(self, log_name, log, model, initial_marking, final_marking, partitioned_log, sample_size,
        ...
        #print("KEEPING ALIGNMENT PARAMS")
        ...
        for i in range(len(self.sample.traces)):
            ...
            upper_bound_total_costs += int(shortest_path) + len(log[self.sample.traces[i]])
    ...
    def _check_for_property(self, log_name, trace, model, initial_marking, final_marking, verbose=False):
        ...
        for idx, is_deviating in enumerate(deviation_contexts):
            ...
            _, features = create_feature_encoding_from_index([current_event], considered_feature_types=feature_encodings.feature_types.EVENT_LEVEL, index_name=self.index_file)
            if is_deviating:
        ...
        _, features = create_feature_encoding_from_index([trace],  considered_feature_types=feature_encodings.feature_types.TRACE_LEVEL, index_name=self.index_file)
    ...
    def remove_trace(self, trace, log_name, verbose=False):
        ...
        data, features = create_feature_encoding_from_index([trace], considered_feature_types=feature_encodings.feature_types.EVENT_LEVEL, index_name=self.index_file)
        ...
        data, features = create_feature_encoding_from_index([trace], considered_feature_types=feature_encodings.feature_types.TRACE_LEVEL, index_name=self.index_file)
        for feature in features:
            if feature in self.partitioned_log:
                self.partitioned_log[feature].remove(trace)
                # if partition is samples completely remove it completely, it cant be sampled anymore
                ...
                #    del self.partitioned_log[feature]
        #TODO n-grams
        #data, features = create_feature_encoding_from_index([trace], considered_feature_types=feature_encodings.feature_types.N_GRAMS, index_name=self.index_file)
        #for feature in features:
        #    if feature in self.partitioned_log:
        #        self.partitioned_log[feature].remove(trace)
