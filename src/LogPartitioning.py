...
class FeatureBasedPartitioning:
    ...
    # TODO use case identifier instead of enumeration
    # TODO remove pruning
    def partition(self, logname, log, verbose=False):
            ...
            for i in range(len(feature_names)):
                if encoded_trace[i] != 0:
                    if feature_names[i] in partitioned_log:
                        partitioned_log[feature_names[i]].append(log[idx])
                    else:
                        partitioned_log[feature_names[i]] = [log[idx]]
